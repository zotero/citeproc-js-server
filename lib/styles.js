...
exports.CslLoader.prototype.scanStyles = function(){
    ...
    if(cslLoader.config.parser == "xml") {
        cslLoader.cslDir = fs.readdirSync(cslLoader.config.cslPath);
    } else if(cslLoader.config.parser == "json") {
        cslLoader.cslDir = fs.readdirSync(cslLoader.config.jsonCslPath);
    } else {
        throw "Unexpected parser type";
    }
    ...
    var extension = ".csl";
    if(cslLoader.config.parser == "json") {
        extension = ".csljson";
    }
    ...
    var renamedFilename = path.join(cslLoader.config.cslPath, 'renamed-styles.json');
...
exports.CslLoader.prototype.resolveStyle = function(styleName){
    ...
    return new Promise(function(resolve, reject){
        ...
        //dependent style we have and have resolved previously
            ...
            var parentStyle = cslLoader.cslDependentShortNames[shortName];
        ...
        //check if dependent style we have, but unresolved
            ...
            var filename = path.join(cslLoader.config.cslDependentPath, shortName + '.csl');
    ...
    return new Promise(function(resolve, reject){
        if(styleUrlObj.host == 'www.zotero.org'){
            ...
            if(cslLoader.cslShortNames[styleUrlObj.shortName] === true){
                ...
                if(cslLoader.config.parser == "json"){
                    var filename = path.join(cslLoader.config.jsonCslPath, styleUrlObj.shortName + '.csljson');
                } else if(cslLoader.config.parser == "xml") {
                    var filename = path.join(cslLoader.config.cslPath, styleUrlObj.shortName + '.csl');
                } else {
                    log.error("CslLoader.fetchIndependentStyle", 'unknown parser type');
                    reject(); 
                }
            ...
            httpGetter.get(url.format(styleUrlObj), function(response) {
...
exports.CslLoader.prototype.readDependent = function(xml){
    ...
    var dStyle = jsdom(xml, {'parsingMode': 'xml'});
