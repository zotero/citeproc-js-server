...
zcite.localesDir = fs.readdirSync(config.localesPath);
...
for(var i = 0; i < zcite.localesDir.length; i++){
    ...
    //log.verbose(config.localesPath + '/' + zcite.localesDir[i], 2);
    zcite.locales[localeCode] = fs.readFileSync(config.localesPath + '/' + zcite.localesDir[i], 'utf8');
}
    ...
    request.on('end', function(){
        try{
            ...
            if(config.memoryUsage == '1'){
                ...
                log.info(JSON.stringify(memoryUsage));
                response.writeHead(200);
