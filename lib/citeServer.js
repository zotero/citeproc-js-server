...
var cslFetcher = zcite.cslFetcher = require(config.cslFetcherPath).cslFetcher;
cslFetcher.init(config);
...
zcite.cacheLoadEngine = function(styleUri, locale) {
    ...
    if (zcite.cachedEngines[cacheEngineString].store instanceof Array) {
        ...
        try {
            ...
            async.waterfall([
                function(callback) {
                    ...
                    if (zcreq.postedStyle) {
                        log.verbose("Style was POSTed, skipping fetchStyleIdentifier");
                        callback(null);
                        return;
                    }

                    var ret = cslFetcher.processStyleIdentifier(zcreq.config.style);
                    if (typeof ret === "string") {  // error
                        callback(ret);
                        return;
                    }
                    
                    zcreq.styleUrlObj = ret;
                    log.silly("zcreq.styleUrlObj: %j", zcreq.styleUrlObj);
                    cslFetcher.resolveStyle(zcreq, callback);
                    return;
                },
                ...
                function(callback) { // fetchStyle
                    log.verbose("Request step: fetchStyle");
                    ...
                    if (typeof zcreq.citeproc != 'undefined') {
                        ...
                        cslFetcher.fetchStyle(zcreq, callback);
                        return;
                    }
                },
