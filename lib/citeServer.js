...
var server = http.createServer(function (request, response) {
    ...
    // Health check
    if (config.healthCheckFile && request.url === '/health') {
        if (!fs.existsSync(config.healthCheckFile)) {
    ...
    request.on('end', function() {
        ...
        Promise.resolve().then(function(){
            ...
            response.end();
            let clientIP = request.socket.remoteAddress.replace(/^::ffff:/, '');
            log.info("request", "#%d %s %s %s %dms", request.requestNumber, clientIP, requestConfig.style, requestConfig.locale, Date.now() - request.startDate);
