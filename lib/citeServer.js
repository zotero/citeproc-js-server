...
/*
  ...
  configuratin object for that module.  It has the following members:
    ...
    - reqItemIDs - An array of item IDs that we should convert.  Can either be
      defined in the POST data (as "itemIDs") or ...
    - reqItemsObj
...
_.extend(config, configDefaults);
...
zcite.respondException = function(err, response, statusCode){
    if(response === undefined){
        log.error("No response passed to respondException");
        return;
    }
...
http.createServer(function (request, response) {
    ...
    request.on('end', function(){
        try{
            ...
            var config = _.extend({}, defaultRequestConfig, parsedQuery);
            ...
            log.silly("zcreq.citationClusters = " + zcreq.citationClusters);
            if (typeof zcreq.citationClusters != 'undefined'){
                addCitationClusters = false;
            }
            
            // Push itemIDs onto array and id referenced object for updateItems and retrieveItem.
